...
# 检查数据库连接
echo "🗄️  检查数据库连接..."
if ! psql -h localhost -U postgres -d ebook2mindmap -c "SELECT 1;" > /dev/null 2>&1; then
    echo "⚠️  数据库连接失败，请检查PostgreSQL配置"
    echo "请确保："
    echo "1. PostgreSQL服务正在运行"
    echo "2. 数据库 'ebook2mindmap' 已创建"
    echo "3. 用户权限配置正确"
    echo ""
    echo "创建数据库命令:"
    echo "psql -U postgres -c \"CREATE DATABASE ebook2mindmap;\""
    echo "psql -U postgres -d ebook2mindmap -f database/init.sql"
fi
